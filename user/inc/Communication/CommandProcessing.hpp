...
namespace Commands{
     ...
     */
    inline ByteCommand Restart_Cmd({0xff, 0xff, 0x00}, restart, ConfirmPolicy::Required);

    /**
     * @brief   Отложенная команда: переход в стадию FooStage.
     */
    inline ByteCommand Set_FooStage_Cmd({0xaa, 0x01, 0x00}, set_FooStage, ConfirmPolicy::Required);

    /**
     * @brief   Отложенная команда: переход в стадию MeasureStage.
     */
    inline ByteCommand Set_MeasureStage_Cmd({0xaa, 0x02, 0x00}, set_MeasureStage, ConfirmPolicy::Required);
